...
const CreditSettlement = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setWithdrawals(rawWD.map(mapWithdrawal));
    } catch {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {loadingLeads ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-red-400 bg-white border border-red-100 rounded-xl">
              ...
              <button
                onClick={fetchAll}
                className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-[9px] font-black uppercase"
              >
                Retry
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredRewards.map(item => (
                <div
                  key={item.ledgerId}
                  className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col gap-4 hover:shadow-md hover:border-blue-200 transition-all group"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="px-2 py-1 bg-blue-50 text-blue-600 border border-blue-100 rounded text-[9px] font-black uppercase tracking-widest">
                        {item.id}
                      </span>
                      <h4 className="mt-2.5 text-sm font-black text-slate-900 uppercase tracking-tight">
                        {item.clientName}
                      </h4>
                    </div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-3 grid grid-cols-2 gap-3 border border-slate-100">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black text-blue-500 uppercase truncate" title={item.service}>
                        {item.service}
                      </p>
                    </div>
          ...
          {loadingWD ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredPayouts.map(item => (
                <div
                  key={item.id}
                  className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col gap-4 hover:shadow-md hover:border-emerald-200 transition-all"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="px-2 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded text-[9px] font-black uppercase tracking-widest">
                        {item.id}
                      </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <section className="p-4 bg-blue-50 border border-blue-100 rounded-xl space-y-3">
                      ...
                      <div className="flex gap-2 pt-1">
                        <a href={`tel:${selectedItem.clientPhone}`}
                          className="flex-1 py-1.5 bg-white rounded text-[7px] font-black uppercase text-center border border-blue-200">
                          Call
                        </a>
                        <a href={`https://wa.me/${selectedItem.clientPhone}`} target="_blank" rel="noreferrer"
