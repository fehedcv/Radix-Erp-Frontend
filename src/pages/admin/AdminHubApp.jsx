...
const AdminHubApp = ({ onLogout }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] font-['Plus_Jakarta_Sans',sans-serif] text-slate-900">
      ...
      {/* 1. FIXED DESKTOP SIDEBAR - Applied safe area for iPad/Tablet usage */}
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-200 sticky top-0 h-screen z-30 overflow-hidden pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
          ...
          <nav className="space-y-1 relative">
            {menuItems.map((item) => (
              <NavLink
                key={item.id}
                to={item.path}
                className={({ isActive }) => `group relative flex items-center gap-3 px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-200 ${
                  isActive 
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    {/* Replaced Framer Motion with standard conditional rendering for performance */}
                    {isActive && (
                      <div 
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        {/* NATIVE APP TOP BAR (Safe Area applied) */}
        <header className="h-[calc(4rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)] flex items-center justify-between px-6 lg:px-8 sticky top-0 bg-[#F8FAFC]/80 backdrop-blur-md z-20">
          <div className="flex items-center gap-4">
              <span className="text-[10px] font-black text-[#007ACC] uppercase tracking-[0.2em]">Dashboard</span>
          </div>
        </header>

        {/* NATIVE SCROLL VIEW (Padding adjusted to clear bottom navigation) */}
        <div className="py-2 px-4 lg:px-8 max-w-[1600px] w-full mx-auto pb-[calc(6rem+env(safe-area-inset-bottom))] lg:pb-12">
           {/* Stripped AnimatePresence for instant snappy navigation */}
           <div>
             <Outlet />
           </div>
        </div>
      ...
      {/* 3. MOBILE BOTTOM NAVIGATION (Safe Area applied) */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-[calc(4.5rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] bg-white border-t border-slate-200 flex justify-between items-center px-2 z-40 shadow-[0_-8px_30px_rgba(0,0,0,0.06)]">
        {menuItems.map((item) => (
          <NavLink 
            key={item.id}
            to={item.path} 
            className={({ isActive }) => `relative flex flex-col items-center gap-1 flex-1 transition-all duration-200 ${isActive ? 'text-[#007ACC]' : 'text-slate-400'}`}
          >
            {({ isActive }) => (
               <>
                ...
                {/* Replaced Framer Motion with standard div */}
                {isActive && (
                  <div className="absolute -top-[12px] w-8 h-1 bg-[#007ACC] rounded-b-full shadow-[0_2px_10px_rgba(0,122,204,0.4)]" />
                )}
               </>
            )}
          </NavLink>
      ...
      {/* SIGN OUT MODAL (Safe Area applied to wrapper) */}
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] bg-slate-900/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-[1rem] p-8 shadow-2xl border border-slate-100 text-center space-y-8 animate-in fade-in zoom-in-95 duration-200">
