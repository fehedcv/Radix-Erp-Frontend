...
const AgentControlApp = () => {
  ...
  const chartConfigs = useMemo(() => {
    ...
    return {
      ...
      performance: {
        series:  [{ name: 'Total Projects', data: leadCountByAgent.map(d => d.y) }],
        options: {
  ...
  return (
    <div className={`font-['Plus_Jakarta_Sans',sans-serif] space-y-4 pt-2 pb-6 transition-colors duration-200 ${isLight ? 'text-[#1A202C]' : 'text-[#F4F5F7]'}`}>
      ...
      <div className="mb-4 px-1">
        <h2 className="text-2xl font-extrabold tracking-tight mb-1">Global Partner Network</h2>
        <p className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Manage IBP access, portfolio performance, and capital distributions.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4 mb-4">
        ...
        <div className={`rounded-3xl p-5 border transition-all duration-200 flex flex-col justify-center ${isLight ? 'bg-[#FFFFFF] border-[#E2E8F0]' : 'bg-[#222938] border-white/10'}`}>
          <div className="flex justify-between items-start mb-2">
            <p className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Active IBPs</p>
        ...
        <div className={`rounded-3xl p-5 border transition-all duration-200 flex flex-col justify-center ${isLight ? 'bg-[#FFFFFF] border-[#E2E8F0]' : 'bg-[#222938] border-white/10'}`}>
          <div className="flex justify-between items-start mb-3">
            <p className={`text-[10px] font-bold uppercase tracking-wider ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Top Performing Partner</p>
          ...
          {topAgent ? (
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className={`text-[9px] font-bold uppercase tracking-wider ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Projects Initiated</p>
              </div>
            </div>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4">
        <div className="lg:col-span-8">
          <ChartCard title="Partner Project Volume" subtitle="Client projects initiated by each partner." isLight={isLight}>
            <Chart options={chartConfigs.performance.options} series={chartConfigs.performance.series} type="bar" height={260} />
          </ChartCard>
        </div>
        <div className="lg:col-span-4">
          <ChartCard title="Account Clearance" subtitle="Active vs Restricted accounts" isLight={isLight}>
          ...
          {filteredAgents.map((agent) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className={`p-3 rounded-2xl border text-center ${isLight ? 'bg-[#F4F5F7] border-[#E2E8F0]' : 'bg-[#131720] border-transparent'}`}>
                  <p className={`text-[8px] font-bold uppercase tracking-wider mb-1 ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Projects</p>
                  <p className="text-sm font-extrabold">{agent.totalLeads}</p>
                </div>
                <div className={`p-3 rounded-2xl border text-center ${isLight ? 'bg-[#F4F5F7] border-[#E2E8F0]' : 'bg-[#131720] border-transparent'}`}>
                  <p className={`text-[8px] font-bold uppercase tracking-wider mb-1 ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Ledger</p>
              ...
              >
                <ArrowUpRight size={14} strokeWidth={2.5} /> View Partner Portfolio
              </button>
            </div>
          ))}
      </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <DossierSection title="Contact Information" icon={<Mail size={14}/>} isLight={isLight} textPrimary={isLight ? 'text-[#1A202C]' : 'text-[#F4F5F7]'} textSecondary={isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}>
                  ...
                  <div className="space-y-6">
                    ...
                    <section className={`border rounded-3xl p-5 md:p-6 relative overflow-hidden flex flex-col justify-center items-center text-center ${isLight ? 'bg-[#F4F5F7] border-[#E2E8F0]' : 'bg-[#1A1A24] border-white/10'}`}>
                      <div className="absolute top-0 right-0 p-4 opacity-10 rotate-12"><Wallet size={120} /></div>
                      <p className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>Ledger Balance</p>
                      ...
                      <button
                        ...
                        className={`relative z-10 w-full py-3.5 rounded-xl border text-[10px] font-bold uppercase tracking-wider transition-all active:scale-95 ${
                          isLight ? 'bg-[#FFFFFF] border-[#E2E8F0] text-[#1A202C] hover:border-[#81B398] hover:text-[#81B398]' : 'bg-[#131720] border-transparent text-[#F4F5F7] hover:border-[#81B398] hover:text-[#81B398]'
                        }`}
                      >
                        Manage Settlements
                    ...
                    <section className={`border rounded-3xl p-5 md:p-6 ${isLight ? 'bg-[#FFFFFF] border-[#E2E8F0]' : 'bg-[#131720] border-white/10'}`}>
                      <h5 className={`text-[10px] font-bold uppercase tracking-wider mb-5 pb-3 border-b flex items-center gap-2 ${isLight ? 'border-[#E2E8F0] text-[#718096]' : 'border-white/10 text-[#9CA3AF]'}`}>
                        <Briefcase size={14} strokeWidth={2.5} className="text-[#81B398]" /> Assigned Projects ({agentLeads.length})
                      </h5>
                      <div className="space-y-3 max-h-[240px] overflow-y-auto pr-2 no-scrollbar">
                        {agentLeads.length > 0 ? agentLeads.map((l) => (
                          ...
                          <p className={`text-[10px] font-bold uppercase tracking-wider text-center py-4 ${isLight ? 'text-[#718096]' : 'text-[#9CA3AF]'}`}>No projects assigned yet.</p>
                        )}
                      </div>
                    </section>
                  </div>
...
const DossierSection = ({ title, icon, children, isLight, textPrimary, textSecondary }) => (
  <section className={`border rounded-3xl p-5 md:p-6 transition-all duration-200 ${isLight ? 'bg-[#FFFFFF] border-[#E2E8F0]' : 'bg-[#131720] border-white/10'}`}>
    <h5 className={`text-[10px] font-bold uppercase tracking-wider mb-5 pb-3 border-b flex items-center gap-2 ${isLight ? 'border-[#E2E8F0] text-[#718096]' : 'border-white/10 text-[#9CA3AF]'}`}>
