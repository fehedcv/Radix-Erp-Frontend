...
const LeadFormModal = ({ isOpen, onClose, initialUnit, onSubmitLead }) => {
  ...
  // --- UPDATED LOGIC TO SAVE DATA ---
  const finalizeSubmission = () => {
    if (onSubmitLead) {
      onSubmitLead(formData); // Sends data back to AgentHub
    }
    setStep('success');
  };
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-8 md:p-12">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <select name="category" required value={formData.category} onChange={handleInputChange} className="w-full pl-14 pr-10 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 appearance-none">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" name="clientName" required value={formData.clientName} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 transition-all" placeholder="Full Name" />
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Phone Number</label>
                      <input type="tel" name="clientPhone" required value={formData.clientPhone} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 transition-all" placeholder="+971..." />
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Client Address</label>
                      <textarea name="clientAddress" required rows="2" value={formData.clientAddress} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 resize-none" placeholder="Location details..."></textarea>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <select name="service" required value={formData.service} onChange={handleInputChange} className="w-full pl-14 pr-10 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 appearance-none disabled:opacity-50" disabled={!selectedBusiness}>
                    ...
                    <div className="space-y-2 flex-grow">
                      ...
                      <textarea name="description" required value={formData.description} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none text-sm font-bold focus:border-indigo-600 h-[180px] resize-none" placeholder="Describe the scope..."></textarea>
                    </div>
            ...
            {step === 'confirm' && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-10 max-w-lg mx-auto">
                ...
                <h3 className="text-2xl font-black uppercase">Confirm Referral</h3>
                <p className="text-sm text-slate-500 mt-2">Are you sure you want to refer <b>{formData.clientName}</b> to <b>{formData.category}</b>?</p>
                <div className="bg-slate-50 p-6 rounded-[2rem] my-8 text-left space-y-2 border border-slate-100">
                  <p className="text-[10px] font-black uppercase text-slate-400">Target Service</p>
                  <p className="font-bold text-slate-900">{formData.service} at {formData.category}</p>
                </div>
                <div className="flex gap-4">
                  <button onClick={() => setStep('form')} className="flex-1 py-4 text-slate-400 font-black uppercase text-[10px] tracking-widest hover:text-slate-900">Go Back</button>
                  <button onClick={finalizeSubmission} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-100 hover:bg-slate-900 transition-all">Yes, Confirm & Send</button>
                </div>
              </motion.div>
            )}
            ...
            {step === 'success' && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12 max-w-md mx-auto">
                ...
                <p className="text-slate-500 mt-4 leading-relaxed">Your referral has been successfully sent to the <b>Business Owner</b> and <b>Vynx Admin</b>.</p>
