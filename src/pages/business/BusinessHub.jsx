...
const BusinessHub = ({ onLogout }) => {
  ...
  // Modern Single Icon Theme Toggle
  const ThemeToggle = () => (
    <button
      onClick={toggleTheme}
      className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
        isLight
          ? 'bg-white/20 text-[#FFFFFF] hover:bg-white/30' // Adapted for visibility on the dark sidebar
    ...
    <div className={`flex h-screen font-['Plus_Jakarta_Sans',sans-serif] overflow-hidden transition-colors duration-300 ${
      ...
      <aside className="hidden lg:flex flex-col w-[260px] h-screen shrink-0 transition-all duration-300">
        ...
        <div className="px-6 py-6 flex flex-col items-center justify-center text-center">
          <div className="w-[70px] h-[70px] rounded-full flex items-center justify-center overflow-hidden mb-3 border-2 border-white/10 bg-white/5 shadow-sm">
            {businessLogo ? (
          ...
          <h2 className="text-sm font-bold tracking-tight w-full truncate text-[#FFFFFF]">
            {businessName}
          </h2>
        ...
        {showLogoutConfirm && (
          ...
          >
            <div 
              ...
              className={`w-full max-w-[320px] p-8 rounded-md shadow-xl text-center border transition-all ${
                isLight ? 'bg-[#FFFFFF] border-[#E2E8F0]' : 'bg-[#222938] border-white/5'
              }`}
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button 
                  onClick={onLogout} 
                  className="py-2.5 bg-[#F0524F] text-[#FFFFFF] rounded-lg text-xs font-semibold hover:bg-[#D44846] transition-all shadow-sm"
                >
                  Confirm
                </button>
              </div>
