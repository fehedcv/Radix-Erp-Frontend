...
const BusinessHubApp = ({ onLogout }) => {
  ...
  return (
    <div className={`flex h-screen font-['Plus_Jakarta_Sans',sans-serif] overflow-hidden transition-colors duration-300 ${
      ...
      {/* 1. SIDEBAR - No Pure White (Added safe areas for tablet/iPad usage) */}
      <aside className={`hidden lg:flex flex-col w-64 h-screen shrink-0 border-r transition-all duration-300 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] ${
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
        ...
        <header className={`h-[calc(3.5rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)] flex items-center justify-between px-6 lg:px-8 sticky top-0 z-20 border-b transition-colors duration-300 ${
        ...
        {/* NATIVE SCROLL VIEW (Padding adjusted to clear bottom bar) */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 pb-[calc(5rem+env(safe-area-inset-bottom))] lg:pb-6 no-scrollbar">
      ...
      {/* 3. MOBILE NAVIGATION (Safe Area applied) */}
      <nav className={`lg:hidden fixed bottom-0 left-0 right-0 h-[calc(4rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] flex justify-around items-center px-4 z-50 border-t transition-colors duration-300 ${
        isLight ? 'bg-[#F8FAFB] border-[#E8ECEF]' : 'bg-[#0F172A] border-white/10'
      }`}>
        {navItems.map((item) => (
          <NavLink 
      ...
      <AnimatePresence>
        {showLogoutConfirm && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] bg-black/20 backdrop-blur-sm">
