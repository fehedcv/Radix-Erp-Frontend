...
if (userData) {
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 pt-[env(safe-area-inset-top)] pb-[calc(5rem+env(safe-area-inset-bottom))]">
  ...
  return (
    <motion.div
      ...
      className={`max-w-[1000px] mx-auto space-y-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(5rem+env(safe-area-inset-bottom))] font-sans px-4 sm:px-6 transition-colors duration-300 ${isLight ? 'text-[#1A1D1F]' : 'text-[#E2E8F0]'}`}
    >
...
<div className={`border rounded-xl p-6 shadow-sm relative overflow-hidden transition-all duration-300 ${
  ...
  <div className="grid gap-6">
    ...
    <div className={`flex items-start gap-4 pt-5 border-t transition-colors ${
      ...
      <AnimatePresence>
        {modal.show && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[600] flex items-center justify-center p-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(1rem+env(safe-area-inset-bottom))]">
            <motion.div
